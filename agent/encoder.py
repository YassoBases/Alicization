...
Architecture (channel widths and embed dim from the ``model`` config section):
Conv(C->c1, 3x3) ReLU -> Conv(c1->c2, 3x3) ReLU -> flatten -> Linear -> embed,
concat intero -> Linear -> embed. Convs are unpadded, so an 11x11 window
shrinks to 7x7 before flattening.
"""
...
class ObsEncoder(nn.Module):
    ...
    def forward(self, grid: torch.Tensor, intero: torch.Tensor) -> torch.Tensor:
        """(B, C, W, W) grid + (B, intero_dim) -> (B, obs_embed_dim) embedding."""
