...
def tiny_cfg(seed: int = 5) -> dict:
    ...
    cfg["device"] = "cpu"
    cfg["model"] = {
        "core": "gru", "core_hidden": 16, "obs_embed_dim": 16,
        "encoder_channels": [4, 8],
    }
    cfg["ppo"].update(
        rollout_length=8, seq_len=4, num_envs=2, episode_length=16,
        num_minibatches=2, update_epochs=1, total_env_steps=10**9,
        anneal_lr=False,
    )
...
def test_gru_hidden_zeroed_at_done_boundary_vs_hand_forward() -> None:
    ...
    """
    torch.manual_seed(0)
    core = GRUCore({"core_hidden": 8}, input_dim=4)
    x = torch.randn(3, 2, 4)
    h0 = torch.randn(2, 8)
    ...
    with torch.no_grad():
        o0 = core.cell(x[0], h0)
        o1 = core.cell(x[1], o0)  # no done at step 0 -> hidden carried
        h2_in = o1 * (1.0 - dones[1]).unsqueeze(-1)
        o2 = core.cell(x[2], h2_in)
    ...
    with torch.no_grad():
        from_zero = core.cell(x[2, 0:1], torch.zeros(1, 8))[0]
    assert torch.allclose(outs[2, 0], from_zero, atol=1e-6)
    # ...while env 1 (no done) was NOT reset.
    assert not torch.allclose(outs[2, 1], core.cell(x[2, 1:2], torch.zeros(1, 8))[0])
...
def test_encoder_output_shape() -> None:
    enc = ObsEncoder(
        {"encoder_channels": [4, 8], "obs_embed_dim": 32},
        grid_channels=8, intero_dim=6, window=11,
    )
...
def test_checkpoint_resume_identical_step_and_loss(tmp_path: Path) -> None:
    ...
    cfg = tiny_cfg()
    steps_per_update = cfg["ppo"]["rollout_length"] * cfg["ppo"]["num_envs"]
